    ...
    return (
      <div className="min-h-screen flex items-center justify-center pt-20">
        <div className="text-center">
          <h2 className="text-2xl font-bold mb-4">{t('newsDetail.notFound')}</h2>
          <Link to="/" className="text-eu-blue hover:underline">{t('newsDetail.returnHome')}</Link>
        </div>
      </div>
    );
  }
          ...
          <div className="grid lg:grid-cols-12 gap-12">
            <div className="lg:col-span-8">
              ...
              <div className="mt-12 flex items-center gap-4 py-8 border-t border-slate-100">
                <span className="text-sm font-bold text-slate-900">{t('newsDetail.share')}</span>
            ...
            <div className="lg:col-span-4">
              <div className="sticky top-32 space-y-8">
                <div className="bg-slate-50 p-8 rounded-3xl border border-slate-100">
                  <h4 className="font-bold text-slate-900 mb-4">{t('newsDetail.projectSummaryTitle')}</h4>
                  <p className="text-sm text-slate-500 leading-relaxed mb-6">
                    {t('newsDetail.projectSummaryText')}
                  </p>
                  <Link to="/" className="text-sm font-black text-eu-blue hover:underline">{t('newsDetail.learnMore')}</Link>
                </div>

                <div className="bg-eu-blue p-8 rounded-3xl text-white shadow-xl shadow-blue-900/20">
                  <h4 className="font-bold mb-4">{t('newsDetail.newsletterTitle')}</h4>
                  <p className="text-white/70 text-sm mb-6">{t('newsDetail.newsletterText')}</p>
                  <input type="email" placeholder={t('newsDetail.emailPlaceholder')} className="w-full px-4 py-3 rounded-xl bg-white/10 border border-white/20 text-white placeholder:text-white/40 mb-4 focus:outline-none focus:ring-2 focus:ring-white/30" />
                  <button className="w-full py-3 bg-white text-eu-blue font-bold rounded-xl hover:bg-slate-100 transition-colors">{t('newsDetail.subscribe')}</button>
                </div>
              </div>
            </div>
          </div>
